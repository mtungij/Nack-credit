...
public function get_customer_data($customer_id){
	$customer = $this->db->query("SELECT c.customer_id,c.blanch_id,c.comp_id,c.empl_id,e.empl_name,c.customer_day FROM tbl_customer c LEFT JOIN tbl_employee e ON e.empl_id = c.empl_id  WHERE c.customer_id = $customer_id");
	  return $customer->row();
}
...
public function get_totalLoanout($customer_id){
	...
	public function get_blanchTransaction($from,$to,$blanch_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT * FROM tbl_prev_lecod p JOIN tbl_customer c ON c.customer_id = p.customer_id JOIN tbl_blanch b ON b.blanch_id = p.blanch_id LEFT JOIN tbl_employee e ON e.empl_id = p.empl_id WHERE date(p.time_rec) between '$from' and '$to' AND p.blanch_id = '$blanch_id' GROUP BY p.blanch_id");
		return $data->result();
	}

	public function get_blanchTransaction_comp($from,$to,$comp_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT * FROM tbl_prev_lecod p JOIN tbl_customer c ON c.customer_id = p.customer_id JOIN tbl_blanch b ON b.blanch_id = p.blanch_id LEFT JOIN tbl_employee e ON e.empl_id = p.empl_id WHERE date(p.time_rec) between '$from' and '$to' AND p.comp_id = '$comp_id' GROUP BY p.blanch_id");
		return $data->result();
	}
...
public function update_account($account_id,$data){
 ...
 public function get_search_dataCustomer($customer_id){
 	$data = $this->db->query("SELECT * FROM tbl_customer c LEFT JOIN tbl_sub_customer sc ON sc.customer_id = c.customer_id LEFT JOIN tbl_account_type at ON at.account_id = sc.account_id  LEFT JOIN tbl_blanch b ON b.blanch_id = c.blanch_id LEFT JOIN tbl_employee e ON e.empl_id = c.empl_id WHERE c.customer_id = '$customer_id'");
 	return $data->row();
 }
